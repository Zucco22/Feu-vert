...
import { C, HEARTS_START, XP_PER_CORRECT } from '../lib/theme';
...
export default function Lesson({ mod, state, onCommit, onExit }) {
  ...
  if (phase === 'intro') {
    return (
      <View style={styles.overlay}>
        <Header onClose={onExit} />
        <ScrollView contentContainerStyle={styles.body}>
          <View style={[styles.badge, { width: 84, height: 84 }]}>
  ...
  return (
    <View style={styles.overlay}>
      <Header onClose={onExit} progress={(qi + (answered ? 1 : 0)) / total} hearts={hearts} />
      <ScrollView contentContainerStyle={styles.body}>
        ...
        {answered ? (
          <View style={styles.explain}>
            <Text
              style={[
                styles.verdict,
                { color: q.options[selected].correct ? C.success : C.danger },
              ]}
            >
              {q.options[selected].correct ? 'Bonne réponse !' : 'Pas tout à fait.'}
            </Text>
...
function Header({ onClose, progress, hearts }) {
  return (
...
const styles = StyleSheet.create({
  ...
  choiceCorrect: { borderColor: C.success, backgroundColor: '#EAF7DE' },
  choiceIncorrect: { borderColor: C.danger, backgroundColor: '#FDE7E7' },
  optLetter: {
    width: 28,
    height: 28,
    borderRadius: 14,
  ...
  explain: { marginTop: 18, padding: 16, borderRadius: 12, backgroundColor: C.surface2 },
  ...
  pct: { fontSize: 56, fontWeight: '800', color: C.text, marginTop: 8 },
  pctLabel: { color: C.textMuted, fontSize: 14 },
